"""Submit the authoritative validation notebook to a Kaggle GPU kernel."""
...
ATTACK_CELL_TAG = "aas-attack-injection"
...
def default_notebook_path() -> Path:
    """Return the authoritative notebook path in this checkout."""

    return _repository_root() / "scripts" / "aas-local-validation.ipynb"
...
def _injection_source(payload: bytes, digest: str) -> list[str]:
    encoded = base64.b64encode(payload).decode("ascii")
    chunks = [encoded[index : index + 100] for index in range(0, len(encoded), 100)]
    lines = [
        "# Generated locally by `jed-validate kaggle run`; do not edit this cell on Kaggle.\n",
    ...
    lines.extend(
        [
            ")\n",
            "attack_bytes = base64.b64decode(ATTACK_PAYLOAD_B64)\n",
            "assert hashlib.sha256(attack_bytes).hexdigest() == ATTACK_SHA256\n",
            "ATTACK_PATH.write_bytes(attack_bytes)\n",
            "print(f'Injected attack.py ({len(attack_bytes)} bytes, sha256={ATTACK_SHA256})')\n",
        ]
    )
    return lines


def kernel_metadata(kernel: str, notebook_name: str, accelerator: str) -> dict[str, Any]:
...
def stage_kernel(
    ...
    """Create an uploadable notebook without modifying either source file."""
    ...
    try:
        ...
        raise KaggleRunnerError(
            f"Cannot read validation notebook {notebook_path}: {error}"
        ) from error
    ...
    if len(tagged_cells) != 1:
        raise KaggleRunnerError(
            f"Validation notebook must contain exactly one `{ATTACK_CELL_TAG}` cell"
        )

    digest = hashlib.sha256(payload).hexdigest()
    tagged_cells[0]["source"] = _injection_source(payload, digest)
    for cell in notebook.get("cells", []):
    ...
    notebook.setdefault("metadata", {})["kaggle"] = {
    ...
    staged_notebook = destination / notebook_path.name
    staged_metadata = destination / "kernel-metadata.json"
    staged_notebook.write_text(
        json.dumps(notebook, indent=1, ensure_ascii=False) + "\n", encoding="utf-8"
    )
    staged_metadata.write_text(
        json.dumps(kernel_metadata(kernel, staged_notebook.name, accelerator), indent=2) + "\n",
        encoding="utf-8",
    )
    return StagedKernel(destination, staged_notebook, staged_metadata, digest)
...
def ensure_kernel_idle(cli: KaggleClient, kernel: str) -> None:
    ...
    if _status_kind(result.stdout) == "active":
        raise KaggleRunnerError(
            ...
            f"`jed-validate kaggle status --kernel {kernel}` instead."
        )
...
def _wait_for_terminal_status(
    ...
    raise KaggleRunnerError(
        ...
        "still be active; use `jed-validate kaggle status`."
    )
...
def run_remote_validation(
    ...
    *,
    ...
    """Stage, run, monitor, and download one authoritative validation."""

    kaggle = cli or KaggleCli()
    notebook = notebook_path or default_notebook_path()
    ...
    with tempfile.TemporaryDirectory(prefix="jed-kaggle-") as temporary:
        ...
        if version is None:
            raise KaggleRunnerError(
                ...
                "use `jed-validate kaggle status` before retrying."
            )
    ...
    try:
        ...
        print(
            ...
            f"Resume with: jed-validate kaggle status --follow --kernel {kernel}\n"
            f"Fetch later with: jed-validate kaggle fetch --kernel {kernel}",
            file=sys.stderr,
        )
        raise
...
def print_summary(summary: Mapping[str, Any], result_directory: Path) -> None:
    ...
    mean = summary.get("local_public_mean")
    if mean is not None:
